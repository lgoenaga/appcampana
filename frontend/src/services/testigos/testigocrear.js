...
import { createTestigo } from "../../routes/testigos";

import { AuthHeaders } from "../../components/authheader";
import { MostrarRol } from "../../components/decodec";

export function CrearTestigo() {
  ...
  const handleOnSubmit = async (event) => {
    ...
    Swal.fire({
      title: "Desea guardar el Testigo? ",
      ...
      try {
        if (result.isConfirmed) {
          data = await createTestigo(testigo, authheader);

          Swal.fire({
            icon: "success",
            title: "Testigo creado",
            showConfirmButton: false,
            timer: 2000,
            didOpen: () => {
              Swal.showLoading();
          ...
          if (result.isDenied) {
            Swal.fire({
              icon: "info",
              title: "Testigo no ha sido creado",
              showConfirmButton: false,
              timer: 2000,
              didOpen: () => {
                Swal.showLoading();
        ...
        if (result.dismiss === "cancel") {
          Swal.fire({
            icon: "error",
            title: "Se ha cancelado la creación del testigo",
            showConfirmButton: false,
            timer: 2000,
          });
          Swal.showLoading();
  ...
  return (
    <>
      ...
      <Container className="button-contactos">
        <Button variant="primary" onClick={handleOnSubmit} className="registrociudadano">
          Guardar
        </Button>
        <Button variant="primary" onClick={pageHome} className="registrociudadano">
          INICIO
        </Button>
      </Container>
    </>
